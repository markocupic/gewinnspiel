...
class ModuleGewinnspielTeilnehmer extends ModuleGewinnspiel
{
       ...
       public function generate()
       {
              ...
              if (strlen($this->Input->get('token')))
              {
                     ...
                     if (!$objCode->next())
                     {
                            ...
                            die(utf8_decode('ungültiges Token'));
                     }
                     if (strlen($this->Input->post('FORM_SUBMIT')))
                     {
                            ...
                            if ($objCode->next())
                            {
                                   ...
                                   {
                                          $_SESSION['gewinnspiel_get_certificate']['error'] = 'Der Gewinn wurde bereits ausbezahlt. Sie können das Zertifikat nicht mehr abrufen!';
                                          $this->reload();
                                   }
                            ...
                            {
                                   // reload page and display the form again
                                   $_SESSION['gewinnspiel_get_certificate']['error'] = 'Der Code passt nicht zum Token. Bitte probieren Sie es nochmals!';
                                   $this->reload();
                            }
                     }
                     else
       ...
       protected function compile()
       {
              ...
              switch ($this->status)
              {
                     ...
                     case 'show_certificate_form':
                            ...
                            {
                                   $this->Template->message = $_SESSION['gewinnspiel_get_certificate']['error'];
       ...
       protected function registerUser()
       {
              ...
              // add member to tl_avisota_recipient
              if ($this->addUserToAvisotaRecipientList)
              {
